...
.popup.show {
	...
	<script type="text/javascript">
		function chkalert(chat_id,freelancer_id){
			sts= confirm("Are you sure you want to delete this message?");
			if(sts){
				document.location.href='delete_message.php?delete='+chat_id+ "&" + 'freelancer=' +freelancer_id;
				return true;
			}
		}
	...
	function save(chat_id, freelancer_id){
    // $('.edit-popup form').submit(function(event) {
        ...
        $.ajax({
            ...
            data: { chat_id: chat_id, freelancer_id: freelancer_id, message: message },
            success: function(data) {
                // Update the message in the chat box
                $('#chatBox').append(data);
                scrollDown();
				document.location.href='home.php?freelancer=' +freelancer_id;
				closeEditPopup(chat_id)
            }
        });
    };
	function star(chatF, freelancer_id){
		...
		$.ajax({
            type: 'POST',
            url: 'edit_message.php',
            data: { chat_id: chatF, freelancer_id: freelancer_id, star: true},
            success: function(data) {
	...
	function unstar(chatF, freelancer_id){
		// $("button").click(function() {
		// 	var star = $("button").val();
		// 	// alert(fired_button);
		// 	console.log("help");
		...
		$.ajax({
            type: 'POST',
            url: 'edit_message.php',
            data: { chat_id: chatF, freelancer_id: freelancer_id, unstar: true},
            success: function(data) {
				console.log("Star action was successful");
            // Example of handling the success, you may need to update UI or alert user
...
<body class="d-flex
    		...
    		<ul id="chatList"
    			...
    			<?php if (!empty($conversations)) { ?>
	    			...
	    			<li class="list-group-item">
	    				<a href="home.php?freelancer=<?=$conversation['user_1']?>"
	    				   class="d-flex
	    				          justify-content-between
                      ...
                      <small>
                        <?php 
                        	$lastChatMessage= lastChat($conversation['freelancer_id'],$_SESSION['user_id'], $conn);
    ...
    # Getting Freelancer data data
  	$chatWith = getFree($_GET['freelancer'], $conn);
  	...
  	$chats = getChats($chatWith['freelancer_id'], $_SESSION['user_id'], $conn);

  	opened($chatWith['freelancer_id'], $conn, $chats);
    };
?>
<?php if(isset($_GET['freelancer'])){?>
    	   ...
    	   <div class="shadow p-4 rounded
    	        ...
    	        <?php 
                     ...
                     foreach($chats as $chat){
                     	if(($chat['user_id'] == $_SESSION['user_id']) && ($chat['type']==1))
                     	{ ?>
						...
						<div class="yousab">
							...
							<div class="dropdown d-inline">
								...
								<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
									<li><a class="dropdown-item" href="javascript:void()" onClick="chkalert(<?php echo $chat_id?>,<?php echo $freelancer_id;?>)" >Delete</a></li>
							...
							<!-- <a href="javascript:void()" onClick="chkalert(<?php echo $chat_id?>,<?php echo $freelancer_id;?>)">delete</a>
							...
							<h3>Edit Message</h3>
								<form onsubmit="event.preventDefault(); save(<?php echo $chat_id?>, <?php echo $freelancer_id?>);">
					...
					<p class="ltext border 
					    ...
					    <small class="d-block">
					    	<?=$chat['created_at']?>
							<?php if($chat['star'] == 0){?>
							 	<button onClick="star(<?php echo $chatF;?>, <?php echo $freelancer_id; ?>)"><i class="fa-regular fa-star" style="color:#ffc100"></i></button>
							 <?php }elseif($chat['star']== 1){?>
								<button onClick="unstar(<?php echo $chatF;?>, <?php echo $freelancer_id; ?>)"><i class="fa-solid fa-star" style="color:#ffc100"></i></button>
							<?php } ?>	
					    </small>      	
					</p>
					</div>
...
<?php 
if(isset($_SESSION['user_id']) && isset($_GET['star'])){
    $id_2 = $_SESSION['user_id'];
    
    $sql = "SELECT * FROM chat WHERE user_id = ? AND `star` = 1 AND `type` = 1 ORDER BY chat_id DESC";
 ...
 $localhost= "localhost";
	...
	$selectNum="SELECT * FROM chat where type=1 and star=1 and user_id= '$user_id'";
	...
	$(document).ready(function(){
      ...
      $("#sendBtn").on('click', function(){
      	...
      	$.post("app/ajax/insert.php",
      		   {
      		   	message: message,
      		   	freelancer_id: <?=$chatWith['freelancer_id']?>
      		   },
					...
					<div class="yousab">
                        <p class="rtext align-self-end
							...
							<div class="dropdown d-inline">
								<button class="fa-solid fa-ellipsis-vertical" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false"></button>
								<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
									<li><a href="javascript:void()" onclick="chkalert(${response.chat_id}, ${response.user_id})">delete</a></li>
									<li><a href="javascript:void()" onclick="openEditPopup(${response.chat_id}, '${response.message}')">edit</a></li>

								</ul>
							</div>
                            <div class="popup edit-popup" id="edit-popup-<?php echo $chat_id?>">
                            <h3>Edit Message</h3>
                                <form onsubmit="event.preventDefault(); save(${response.chat_id}, ${response.user_id});">
